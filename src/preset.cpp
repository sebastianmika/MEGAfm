...
byte getByte() {
	...
	if (index < 3950) {
		...
		data = EEPROM.read(indexInt);
	} // internal eeprom
	else {
		data = eRead(index - 3950);
	} // overflow to external eeprom
...
void store(byte input) {
	if (index < 3950) {
		int indexInt = int(index);
		EEPROM.write(indexInt, input);
	} // internal eeprom
	else {
		eWrite(index - 3950, input);
	} // overflow to external eeprom
